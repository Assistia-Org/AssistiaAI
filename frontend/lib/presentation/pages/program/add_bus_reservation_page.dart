...
class _AddBusReservationPageState extends ConsumerState<AddBusReservationPage> {
  ...
  void _fillFromAI(Map<String, dynamic> d) {
    ...
    _companyCtrl.text = d['company'] ?? d['operator'] ?? '';
    _busNoCtrl.text = d['bus_no'] ?? d['sefer_no'] ?? '';
    _seatCtrl.text = d['seat'] ?? d['koltuk'] ?? '';
    _pnrCtrl.text = d['pnr'] ?? '';
    _passengerCtrl.text = d['passenger'] ?? d['name'] ?? '';
    if (d['date'] != null) {
      try {
        setState(() =>
    ...
    _parseTime(d['departure_time'], (t) => setState(() => _departureTime = t));
    _parseTime(d['arrival_time'], (t) => setState(() => _arrivalTime = t));
    setState(() {});
  }
  ...
  Future<void> _submit() async {
    ...
    final reservation = Reservation(
      category: 'bus',
      title: 'Otobüs: ${_departureCtrl.text} → ${_arrivalCtrl.text}',
      details: details,
      startDate: dep,
      endDate: arr,
    ...
    try {
      ...
      if (mounted) {
        ...
        _showSnack('Otobüs rezervasyonu eklendi!');
      }
    } catch (e) {
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _sectionLabel('GÜZERGAH', Icons.directions_bus_rounded, _accent),
                    const SizedBox(height: 12),
                    _row2(
                      _textField(_departureCtrl, 'Kalkış', 'Ankara, İstanbul...', Icons.trip_origin_rounded, required: true),
                      _textField(_arrivalCtrl, 'Varış', 'İzmir, Antalya...', Icons.location_on_rounded, required: true),
                    ),
                    const SizedBox(height: 12),
                    _row2(
                      _textField(_companyCtrl, 'Firma', 'Kamil Koç, Metro...', Icons.business_rounded),
                      _textField(_busNoCtrl, 'Sefer No', '1234', Icons.numbers_rounded),
                    ),
                    const SizedBox(height: 12),
                    _dateTile(),
                    const SizedBox(height: 12),
                    _row2(
                      _timeTile('Kalkış Saati', _departureTime, (t) => setState(() => _departureTime = t)),
                      _timeTile('Varış Saati', _arrivalTime, (t) => setState(() => _arrivalTime = t)),
                    ),
                    const SizedBox(height: 24),
                    _sectionLabel('KOLTUK & BİLET', Icons.airline_seat_recline_normal_rounded, const Color(0xFFEC4899)),
                    const SizedBox(height: 12),
                    _row2(
                      _textField(_seatCtrl, 'Koltuk No', '12A', Icons.event_seat_rounded),
                      _textField(_pnrCtrl, 'PNR / Bilet No', 'ABC123', Icons.qr_code_rounded),
                    ),
                    const SizedBox(height: 12),
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [_accent.withValues(alpha: 0.12), const Color(0xFFEC4899).withValues(alpha: 0.08)],
          ...
          Padding(
            ...
            child: _isAnalyzing
                ...
                : Row(children: [
                    ...
                    const SizedBox(width: 12),
                    Expanded(child: _aiBtn('Fotoğraf', Icons.add_photo_alternate_rounded, const Color(0xFFEC4899), _pickImage)),
                  ]),
          ),
        ],
      ]),
  ...
  Widget _dateTile() => _tappable(
        ...
        child: Row(children: [
          ...
          Text('Kalkış Tarihi', style: GoogleFonts.inter(color: Colors.white54, fontSize: 13)),
          const Spacer(),
